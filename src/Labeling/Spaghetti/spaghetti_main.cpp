...
int main() {
  ...
  pixel_set block_positions{{"P", {-2, -2}},
                            {"Q", {+0, -2}},
                            {"R", {+2, -2}},
                            {"S", {-2, +0}},
                            {"x", {+0, +0}}};
  GeneratePointersConditionsActionsCode(
      rs, GenerateConditionActionCodeFlags::NONE,
      GenerateActionCodeTypes::LABELING, block_positions);
  auto conditions =
      GenerateConditions(rs, GenerateConditionActionCodeFlags::NONE);
  auto actions =
      GenerateActions(rs, GenerateConditionActionCodeFlags::NONE,
                      GenerateActionCodeTypes::LABELING, block_positions);

  // 7) Generate the C/C++ code taking care of the names used
  //    in the Grana's rule set GranaRS
  fh.GenerateCode(conditions, actions, BeforeMainShiftTwo);
