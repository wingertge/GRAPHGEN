...
/*BinaryDrag<conact>::node* MergeEquivalentTreesRec(BinaryDrag<conact>::node* a,
   BinaryDrag<conact>::node* b, std::unordered_map<BinaryDrag<conact>::node*,
   BinaryDrag<conact>::node*> &merged)
    {
    ...
    void DeleteTreeRec(BinaryDrag<conact>::node* n,
   std::unordered_map<BinaryDrag<conact>::node*, bool> &deleted)
    {
...
/*void PrintTreeRec(std::ostream& os, BinaryDrag<conact>::node* n,
   std::set<BinaryDrag<conact>::node*>& visited, int tab = 0) { os <<
   std::string(tab, '\t');
  ...
  void MergeSpecialLeavesRec(BinaryDrag<conact>::node *n) {
    ...
    if (left->isleaf() && right->isleaf()) {
      if (left->data.eq(right->data)) {
        // In this case the current node becomes a leaf. The action associated
        // to this leaf will be the intersection of the actions of its children.
        n->data = left->data;
...
private:
  ...
  std::unordered_map<BinaryDrag<conact>::node *,
                     std::vector<BinaryDrag<conact>::node *>>
      parents_;
...
public:
  ...
  // Yet another function to populate visited nodes/leaves and parents node data
  // structure
  ...
  void CompressLeaves() {
    ...
    std::unordered_set<BinaryDrag<conact>::node *>
        already_updated; // Store the leaves for which the parents have already
                         // been updated
    // For each actually used leaf
    for (auto &i : visited_leaves_) {
      // Skip multiple actions leaves
      if (i->data.actions().size() != 1 ||
          already_updated.find(i) != end(already_updated)) {
        continue;
      }

      // Compare the current leaf's action with the actions of all the others
      // leaves and merge them (updating parents) if there is a non-empty
      // intersection and the next tree ids are the same
      for (auto &j : visited_leaves_) {
        ...
        if ((i->data.action & j->data.action) != 0 &&
            i->data.next == j->data.next) {
          // Merge required!
        ...
        // DrawDagOnFile("current_dag", bd_, DrawDagFlags::WITH_NEXT |
        // DrawDagFlags::WITH_ROOT_ID);
      }
    }
  }
};

/** @brief This enum class defines the available flags for the DragCompression
 * class. */
DEFINE_ENUM_CLASS_FLAGS(
    DragCompressorFlags, NONE = 0, /**< @brief No flags */
    PRINT_STATUS_BAR =
        1, /**< @brief Whether to print a sort of progress bar or not */
    IGNORE_LEAVES =
        2, /**< @brief Whether to ignore leaves or not during the compression.
                       Please note that compressing the leaves will
              significantly increase the total execution time without improving
              the final compression result in anyway. */
    SAVE_INTERMEDIATE_RESULTS = 4, /**< @brief Whether to delete or not the dot
                                      code used to draw the drag */
)
...
public:
  ...
  // BinaryDrag
  // Iteration is the early stopping criteria and represents the number of
  // "iteration" after a new optimal is found. -1 means no early stopping and
  // it's the dafult
  DragCompressor(
      BinaryDrag<conact> &bd, int iterations = -1,
      DragCompressorFlags flags = DragCompressorFlags::PRINT_STATUS_BAR |
                                  DragCompressorFlags::IGNORE_LEAVES) {
    early_stopping_active_ = iterations != -1;
    iterations_left_ = iterations_max_ = iterations;
    TLOG(
        "Compressing BinaryDrag", RemoveEqualSubtrees{bd}; do {
          changes_ = false;
  ...
  // LineForestHandler
  DragCompressor(
      LineForestHandler &lfh, int iterations = -1,
      DragCompressorFlags flags = DragCompressorFlags::PRINT_STATUS_BAR |
                                  DragCompressorFlags::IGNORE_LEAVES) {
    early_stopping_active_ = iterations != -1;
    iterations_left_ = iterations_max_ = iterations;
    ...
    TLOG(
        msg, RemoveEqualSubtrees{lfh.f_}; do {
          changes_ = false;
    ...
    for (auto &f : lfh.end_forests_) {
      progress_counter_ = 0;
      ResetIterations();
      msg = conf.algorithm_name_ + " - compressing end forest #" +
            std::to_string(fn++) + "\n";
      TLOG(
          msg, RemoveEqualSubtrees{f}; ResetBest(); do {
            changes_ = false;
...
private:
  ...
  struct MergeEquivalentTreesAndUpdate {
    ...
    std::unordered_map<BinaryDrag<conact>::node *,
                       std::vector<BinaryDrag<conact>::node *>> &parents_;

    MergeEquivalentTreesAndUpdate(
        BinaryDrag<conact>::node *a, BinaryDrag<conact>::node *b,
        std::unordered_map<BinaryDrag<conact>::node *,
                           std::vector<BinaryDrag<conact>::node *>> &parents)
        : parents_{parents} {
      MergeEquivalentTreesAndUpdateRec(a, b);
    }

    void MergeEquivalentTreesAndUpdateRec(BinaryDrag<conact>::node *a,
  ...
  void FastDragOptimizerRec(BinaryDrag<conact> &bd, DragCompressorFlags flags) {
    ...
    bool save_intermediate_results =
        flags & DragCompressorFlags::SAVE_INTERMEDIATE_RESULTS;
    ...
    // Then we order subtrees which have equivalent from the deepest to the
    // least deep. This step is somehow useless since later all the possible
    // merges are tested. Ordering the tree is useful to find, in some cases,
    // the best or pseudo optimal solution earlier, so that when the process
    // does not end in "good time" we still have a good solution/compression.
    sort(begin(trees), end(trees),
         [](const CollectDragStatistics::STreeProp &a,
            const CollectDragStatistics::STreeProp &b) {
    ...
    for (size_t i = 0; i < trees.size(); ++i) {
      ...
      for (j = i + 1; j < trees.size(); ++j) {
        if (trees[i].equivalent(trees[j])) {
          ...
          // copy.
          std::vector<BinaryDrag<conact>::node *> tracked_nodes{trees[i].n_,
                                                                trees[j].n_};
          ...
          MergeEquivalentTreesAndUpdate(tracked_nodes[0], tracked_nodes[1],
                                        cds.parents_);
    ...
    if (no_eq) {
      ...
      if (bds.Nodes() < best_nodes_) {
        ...
        if (print_status_bar) {
          std::cout << "\r" << progress_counter_ << " - nodes: " << bds.Nodes()
                    << "; leaves: " << bds.Leaves() << "\n";
        }
      }
    }
