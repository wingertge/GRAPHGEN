...
/** @brief This class stores the configuration data loaded from file. All data
are placed in the config global variable that can be accessed anywhere in the
code.
...
struct ConfigData {
  ...
  std::string config_file_ =
      "config.yaml"; /**< Name of the configuration file */
  ...
  // Code
  std::string code_suffix_ = "_code.rs";
  ...
  ConfigData(std::string &algorithm_name, const std::string &mask_name,
             bool use_frequencies = false);

  // Dot code
  std::filesystem::path GetDotCodePath(const std::string &out_base_name) {
    return algorithm_output_path_ /
           std::filesystem::path(out_base_name + dotcode_suffix_);
  }

  // Dot output
  std::filesystem::path GetDotOutPath(const std::string &out_base_name) {
    return algorithm_output_path_ /
           std::filesystem::path(out_base_name + dot_output_format_);
  }

  // Forest code
  std::filesystem::path GetForestCodePath(const std::string &out_base_name) {
    return algorithm_output_path_ /
           std::filesystem::path(algorithm_name_ + "_" + out_base_name +
                                 forestcode_suffix_);
  }

  // Forest code
  std::filesystem::path GetForestLabelPath() {
    return algorithm_output_path_ /
           std::filesystem::path(algorithm_name_ + forestlabels_suffix_);
  }

  // Frequencies cache
  std::filesystem::path
  GetFrequenciesPath(const std::string &datasets_name) const {
    return algorithm_output_path_ /
           std::filesystem::path(algorithm_name_ + "_" + datasets_name +
                                 frequencies_suffix_);
  }

  // Get ODT path with custom suffix (used in conjunction with frequencies)
  std::filesystem::path
  GetCustomOdtPath(const std::string &custom_suffix) const {
    return algorithm_output_path_ /
           std::filesystem::path(algorithm_name_ + "_" + custom_suffix +
                                 odt_suffix_);
  }
