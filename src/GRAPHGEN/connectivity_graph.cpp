...
// This function creates and initializes an adjacencies graph with name and
// values starting from a pixel_set
graph MakeAdjacencies(const pixel_set &ps, const Connectivity conn) {
  graph g(ps.size());
  ...
  for (size_t i = 0; i < g.size(); ++i) {
    for (size_t j = i; j < g.size(); ++j) {
      auto adjacent = conn == Connectivity::FOUR
                          ? (ps[i].GetDx() == ps[j].GetDx() ||
                             ps[i].GetDy() == ps[j].GetDy())
                          : ChebyshevDistance(ps[i], ps[j]) <= 1;
      g[i][j] = g[j][i] = adjacent;
    }
  }
...
static void MakeConnectivitiesRec(size_t i, size_t j, graph &g, const graph &ag,
...
static void
MakeConnectivitiesRecSpecial(size_t i, size_t j, graph &g, const graph &ag,
                             std::set<size_t> &visited,
...
std::ostream &operator<<(std::ostream &os, const graph &g) {
  ...
  for (size_t i = 0; i < g.arcs_.size(); ++i) {
    os << g.nodes_[i] << " ";
    copy(begin(g.arcs_[i]), end(g.arcs_[i]),
...
template <size_t N> struct less {
  bool operator()(const std::bitset<N> &lhs, const std::bitset<N> &rhs) const {
    return lhs.to_string() < rhs.to_string();
  }
};

// This function generates all possible actions, avoiding useless ones such as
// merges between adjacent pixels
std::vector<std::string>
GenerateAllPossibleLabelingActions(const graph &ag, const Connectivity conn) {
  auto nconds = ag.size();
  auto nrules = 1u << nconds;
  std::set<std::bitset<128>, less<128>>
      actions_set; // Set of actions described as list of pixels to be merged
...
// This function generates all possible actions, avoiding useless ones such as
// merges between adjacent pixels. This version allows to consider reference
// pixel with a name different from x
std::vector<std::string>
GenerateAllPossibleLabelingActions(const graph &ag,
...
void PrintActionsSet(std::set<std::bitset<128>, less<128>> &to_print,
                     std::ostream &os) {
...
//// This function generates all possible actions, avoiding useless ones such as
/// merges between adjacent pixels and considering multiple pixels inside the
/// mask
// std::vector<std::string>
// GenerateAllPossibleLabelingActionsGivenTheSetOfPixelToBeLabeled(const graph&
// ag, const std::vector<std::string>& to_be_labeled_pixels)
//{
//     auto nconds = ag.size();
//     auto nrules = 1u << nconds;
//
//     // Vector of set of actions described as pixels to be merged (bitmapped).
//     The vector contains as many sets as the number of
//     // pixels to be labeled
//     std::vector<std::set<std::bitset<128>, less<128>>>
//     actions_set(to_be_labeled_pixels.size());
//     std::vector<std::vector<std::string>>
//     actions(to_be_labeled_pixels.size());
//
//     //auto posx = ag.rnodes_.at("x");
...
//                     for (size_t lss = 0 + i; lss <
//                     to_be_labeled_pixels.size(); ++lss) {
//                         if (j == posp[lss]) {
...
//                 std::string es = actions[0][e], gs = actions[1][g], is =
//                 actions[2][i];
...
//     std::copy(final_actions.begin(), final_actions.end(),
//     return_val.begin());
//
//     return return_val;*/
...
GenerateAllPossibleLabelingActionsGivenTheSetOfPixelToBeLabeled(
    const graph &ag, const std::vector<std::string> &to_be_labeled_pixels,
    rule_set &rs) {
  ...
  for (int i = 0; i < static_cast<int>(nrules); ++i) {
    ...
    if (r["e"]) {
      graph cg = MakeConnectivitiesSpecial(
          lag, std::vector<std::string>{"e", "g", "i"});
      connectivity_mat con(rs.conditions);
      con.data_ = cg.arcs_;

      MultiMergeSet mse(con, std::vector<std::string>({"e", "g", "i"}),
                        std::string("e"));
      mse.BuildMergeSet();
    ...
    if (r["g"]) {
      graph cg =
          MakeConnectivitiesSpecial(lag, std::vector<std::string>{"g", "i"});
      connectivity_mat con(rs.conditions);
      con.data_ = cg.arcs_;
      MultiMergeSet msg(con, std::vector<std::string>({"g", "i"}),
                        std::string("g"));
      msg.BuildMergeSet();
    ...
    for (const auto &ae : e_actions) {
      for (const auto &ag : g_actions) {
        for (const auto &ai : i_actions) {
          ...
          if (found != std::string::npos && es != "nothing" &&
              es != "e<-newlabel") {
            gs = "g<-e";
          }
          ...
          if (found != std::string::npos && gs != "nothing" &&
              gs != "g<-newlabel") {
            is = "i<-g";
          }
