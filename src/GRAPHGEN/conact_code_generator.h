...
DEFINE_ENUM_CLASS_FLAGS(
    GenerateConditionActionCodeFlags, NONE = 0, /**< @brief No flags */
    CONDITIONS_WITH_IFS =
        (1 << 0), /**< @brief Whether to add if statements or not when
                     generating conditions code. They serve to check if the
                     pixel we want to check is inside the image or not. All the
                     algorithms that make use of prediction can avoid these
                              checks because they are inbuilt in the forest. */
    ACTIONS_WITH_CONTINUE = (1 << 1), /**< @brief Whether to add continues at
                                         the end of each action or not */
)

/** @brief This is the enum class that defines the available algorithms types
for the GenerateActionsCode function.
*/
DEFINE_ENUM_CLASS_FLAGS(
...
names contains the position in the labels image corresponding to the names used
in labeling actions. It is necessary to handle blocks names and defaults to mask
pixel set if not provided.
...
// This is the version for CTBE algorithm. It is very raw and it is based on the
// previous versione of graphgen
