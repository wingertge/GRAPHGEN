...
ConfigData::ConfigData(string &algorithm_name, const string &mask_name,
                       bool use_frequencies)
    : algorithm_name_{algorithm_name}, mask_name_{mask_name} {
  YAML::Node config;
  try {
  ...
  if (config["paths"]["input"]) {
    ...
    if (config["datasets"]) {
      ...
      generate(datasets_path_.begin(), datasets_path_.end(),
               [this, datasets_it = datasets_.begin()]() mutable {
                 return global_input_path_ / path(*datasets_it++);
               });
    }
  }
  ...
  if (config["paths"]["output"]) {
    ...
    rstable_path_ =
        algorithm_output_path_ / path(algorithm_name + rstable_suffix_);

    // Tree / Forest / Dag (.inc)
    treecode_path_ =
        algorithm_output_path_ / path(algorithm_name + treecode_suffix_);
    forestcode_path_ =
        algorithm_output_path_ / path(algorithm_name + forestcode_suffix_);
    treedagcode_path_ =
        algorithm_output_path_ / path(algorithm_name + treedagcode_suffix_);
    forestdagcode_path_ =
        algorithm_output_path_ / path(algorithm_name + forestdagcode_suffix_);
    ...
    chaincode_rstable_path_ =
        global_output_path_ / path(chaincode_rstable_filename_);
  ...
  if (config["dot"]["background"]) {
    dot_background_color_ =
        "\"" + config["dot"]["background"].as<string>() + "\"";
  } else {
    cout << "WARNING: missing dot background color,  " + dot_background_color_ +
                " will be used.\n";
  }
  ...
  if (config["dot"]["ranksep"]) {
    ...
    std::cout << "WARNING: missing dot ranksep, " + dot_ranksep_ +
                     " will be used.\n";
  }
