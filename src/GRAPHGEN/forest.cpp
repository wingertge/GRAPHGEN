...
LineForestHandler::LineForestHandler(const BinaryDrag<conact> &bd,
  ...
  // Setup next_tree_ for holding a reference to the start tree in first
  // position
  ...
  // For each tree we need to create at least one end tree with end line
  // constraints. With BBDT for example we need two end trees for each tree for
  // example. This is explained below representing borderline cases:
  //
  //             -4  -3  -2  -1 | w
  //                            |
  //     +-------+-------+-------+
  ...
  //                            |
  {
    ...
    // Initialize the mapping between main trees (including the start line tree)
    // and end trees
    main_end_tree_mapping_ = vector<vector<size_t>>(
        end_forests_.size(), vector<size_t>(f_.roots_.size()));
    for (auto &etm : main_end_tree_mapping_) {
      iota(etm.begin(), etm.end(), 0);
    }
    end_next_tree_ = main_end_tree_mapping_;

    // For each tree in the end forests the value of the next tree is set to
    // uint32_t max value max value can be replaced with any value, but all end
    // trees must share the same fake next to be able to delete equal (useless)
    // end trees.
...
//       0/  \1
//       5    ...
// these useless condition may appears after the execution of
// CreateReducedTrees.
...
bool LineForestHandler::RemoveEqualEndTrees() {
  ...
  for (size_t i = 0; i < end_forests_.size(); ++i) {
    changed |= RemoveTrees(EqualTrees, end_next_tree_[i], end_forests_[i], true,
                           main_end_tree_mapping_[i]);
  }
  return changed;
}
...
bool LineForestHandler::RemoveTrees(
    bool (*FunctionPtr)(const BinaryDrag<conact>::node *n1,
                        const BinaryDrag<conact>::node *n2),
    vector<size_t> &next_tree, BinaryDrag<conact> &f, bool are_end_trees,
  ...
  f.roots_.erase(remove(begin(f.roots_), end(f.roots_), nullptr),
                 end(f.roots_));
...
// Initializes leave's next trees (drag) of a tree (drag) with sequential
// values.
...
// Perform tree pruning by removing useless nodes. Useless nodes are identified
// looking at given constraints
BinaryDrag<conact>::node *
LineForestHandler::Reduce(const BinaryDrag<conact>::node *n,
                          BinaryDrag<conact> &t, const constraints &constr) {
  if (n->isleaf()) {
    ...
    if (it != end(constr)) {
      ...
      return t.make_node(n->data, Reduce(n->left, t, constr),
                         Reduce(n->right, t, constr));
    }
  }
}